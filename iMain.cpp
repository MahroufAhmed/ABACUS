...
int programstate = 4; // 0-main menu  1- arithmetic 2-scientific 3-logical 4- welcome screen 5-credi 6-instruction
...
char bg[10][70] = {"bgs\\bg.bmp", "bgs\\bgari.bmp", "bgs\\bgsci.bmp", "bgs\\bglogic.bmp","bgs\\first.bmp","bgs\\CREDITS.bmp","bgs\\INSTRUCTIONS.bmp"};
...

{
	if (programstate == 0)
	{
		...
		if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
		{
			...
			else if(mx>0 && mx<125 && my>0 && my<500 - 435){
			   programstate = 4;
			   PlaySound("sound\\click.wav", NULL, SND_ASYNC||SND_LOOP);
		       Sleep(300);
		       PlaySound("sound\\bgmusic.wav", NULL, SND_ASYNC||SND_LOOP);
			}
		}
	}
	...
	else if (programstate == 3)
	{
		...
		if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN){
			//button click on first screen
			if(mx>209 && mx<536 && my>500-240 && my<500 - 192){
				programstate = 0;
				PlaySound("sound\\click.wav", NULL, SND_ASYNC||SND_LOOP);
		        Sleep(300);
		        PlaySound("sound\\bgmusic.wav", NULL, SND_ASYNC||SND_LOOP);
			}
			else if(mx>209 && mx<536 && my>500-310 && my<500 - 262){
				programstate = 5;
				PlaySound("sound\\click.wav", NULL, SND_ASYNC);
			}
			else if(mx>209 && mx<536 && my>500-388 && my<500 - 338){
			    programstate = 6;
				PlaySound("sound\\click.wav", NULL, SND_ASYNC);
			}
		}
...
void arithmeticinput(int mx, int my)
{
	...
	else if (mx >= 443 && mx <= 443 + ArithmeticNormalButtonWidth && my >= 500 - 412 && my <= 500 - 412 + ArithmeticNormalButtonHeight)
	{
		if (isAnsClicked)
		{
			resetexpression();
...
void calculate()
{
	...
	for (i = 0; i < expsize; i++)
	{
		...
		else
		{
			...
			else if ((expression[i] == 'N' && expression[i + 1] == 'O' && expression[i + 2] == 'T'))
			{
				operation = 'm'; // logical NOT
			}
			else if ((expression[i] == 'c' && expression[i + 1] == 'o'))
			{
				operation = 'w'; // for cosine operator
			}
			else if ((expression[i] == '&' && expression[i + 1] == '&'))
			{
				operation = 'W'; // for logical and
			}
			else if ((expression[i] == '|' && expression[i + 1] == '|'))
			{
				operation = 'Q'; // for logical or
			}
			else if ((expression[i] == '^' && expression[i + 1] == '^'))
			{
				operation = 'Z'; // for logical XOR
			}
			else
	...
	if (operationclicked == 0)
	{
		if (operation == '+')
		{
			...
			ans = a + b;
			operationclicked++;
			snprintf(finalans, sizeof(finalans), "%g", ans);
		}
		else if (operation == '-')
		{
			float a, b, ans;
		...
		else if (operation == '%')
		{
			...
			{
				ans = (int)a % (int)b;
				snprintf(finalans, sizeof(finalans), "%d", (int)ans);
			}
			else
			{
		...
		else if (operation == 'H')
		{
			...
			arr2[16 - existing_digit] = 0;

			strcat(arr2, arr);
			strcpy(finalans, arr2);
		}
		else if (operation == 'O')
		{
			int a, b, ans;
		...
		else if (operation == '!')
		{
			...
			operationclicked++;
			if (ans)
				strcpy(finalans, "True");
			else
				strcpy(finalans, "False");
		}
		else if (operation == '>')
		{
			int a, b, ans;
...
char *toHex(int dno)
{
	static char hex[32]; // Assuming 20 characters is sufficient
	int i = 0;
	while (dno != 0)
	{
		int remainder = dno % 16;
		char hexDigit;
		if (remainder < 10)
			hexDigit = remainder + '0'; // Convert to ASCII digit
		else
			hexDigit = remainder - 10 + 'A'; // Convert to ASCII uppercase letter
		hex[i++] = hexDigit;
		dno = dno / 16;
	}
	if (i == 0)
	{
		hex[i++] = '0'; // Special case for 0
	}
	hex[i] = '\0';
	// Reverse the string
	int start = 0;
